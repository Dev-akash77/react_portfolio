      ...
      <div className="container mx-auto px-4">
        ...
        >
          {allClient.map((client, index) => (
            <SwiperSlide
              key={index}
              className="flex items-center justify-center h-[28rem] flex-col rounded-3xl overflow-hidden bg-slate-100"
            >
              <div className="h-1/2 bg-[#ff1700] w-full cc testimonial_up rounded-bl-3xl">
                <div className="bg-white p-[.2rem] overflow-hidden rounded-full cc">
                  <img
                    src={client.image}
                    alt={client.name}
                    className="w-[6rem] h-[6rem] rounded-full border-[.15rem] border-[#ff1700] object-cover "
                  />
                </div>
              </div>
              <div className="h-1/2 w-full">
                ...
                <div className="cc">
                  <button className="px-2 py-1 border-0 outline-none bg-[#ff1700] rounded-md text-lg font-medium text-white mt-5 duration-150 hover:scale-105">View More</button>
                </div>
              </div>
            </SwiperSlide>
          ))}
